                      ...
                      <div className="flex flex-wrap gap-2">
                        {permissions[0] === '*' ? (
                          ...
                          permissions.map((perm) => (
                            <span key={perm} className="px-2 py-1 text-xs bg-blue-500/10 text-blue-500 rounded">
                              {perm.replace(/_/g, ' ')}
                            </span>
                          ))
                        )}
                      ...
                      <div key={role.name} className="p-4 bg-muted/30 rounded-lg">
                        ...
                        <div className="flex flex-wrap gap-2 mt-2">
                          {role.permissions.map((perm) => (
                            <span key={perm} className="px-2 py-1 text-xs bg-green-500/10 text-green-500 rounded">
                              {perm.replace(/_/g, ' ')}
                            </span>
                          ))}
                        </div>
...
function ToggleRow({
  ...
  return (
    <div className={`flex items-center justify-between p-4 rounded-lg ${
      variant === 'warning' ? 'bg-yellow-500/10 border border-yellow-500/20' : 'bg-muted/30'
    } ${disabled ? 'opacity-50' : ''}`}>
      ...
      <button
        ...
        disabled={disabled}
        className={`ml-4 p-1 rounded-lg transition ${
          disabled ? 'cursor-not-allowed' : 'cursor-pointer'
        }`}
      >
        {checked ? (
          <ToggleRight className={`h-8 w-8 ${variant === 'warning' ? 'text-yellow-500' : 'text-primary'}`} />
        ) : (
